...
WifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiPhy")
    ...
    .AddAttribute ("ChannelWidth",
                   ...
                   MakeUintegerChecker<uint16_t> (2, 160))
...
WifiPhy::WifiPhy ()
  : m_txMpduReferenceNumber (0xffffffff),
    ...
    m_existLrWpanPhy (false), //edited by weidong
...
//edited by weidong
void
WifiPhy::SetLrWpanPhy(Ptr<LrWpanPhy> LrWpanPhy)
{
  m_existLrWpanPhy = true;
  m_LrWpanPhy = LrWpanPhy;
//  LrWpanPhyEnumeration currentLrwpanState = m_LrWpanPhy->GetCurrentState();
  m_state->SetLrWpanPhy(m_LrWpanPhy);
}
...
WifiPhy::SetChannelWidth (uint16_t channelwidth)
{
  ...
  NS_ASSERT_MSG (channelwidth == 2 || channelwidth == 5 || channelwidth == 10 || channelwidth == 20 || channelwidth == 22 || channelwidth == 40 || channelwidth == 80 || channelwidth == 160, "wrong channel width value");
...
//      else if (txVector.GetMode() == WifiPhy::GetDsssRate250kbps() ) {
...
WifiPhy::MaybeCcaBusyDuration ()
{
  ...
  //edited by weidong
  if (m_existLrWpanPhy) {
     m_state->SyncLrWpanState(void);
     if (m_state->CheckIsLrWpanCcaBusy (void)){
       Time LrWpanBackoff = Seconds (0.01);
       m_state->SwitchMaybeToCcaBusy (LrWpanBackoff);
     }
  }
}
...
//WifiMode WifiPhy::GetDsssRate250kbps ()
//{
