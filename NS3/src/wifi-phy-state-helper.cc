...
WifiPhyStateHelper::WifiPhyStateHelper ()
  : m_existLrWpanState (false), //edited by weidong
	m_sleeping (false),
    m_isOff (false),
    m_endTx (Seconds (0)),
    m_endRx (Seconds (0)),
...
{
  m_lrwpan_trxState = IEEE_802_15_4_PHY_TRX_OFF;
...
///*edited by weidong, but const function cannot change the member m_lrwpan_trxState, however, SyncLrWpanState will change*/
  else if (m_endCcaBusy > Simulator::Now () || CheckIsLrWpanCcaBusy())
...
//Edited by Weidong, to set the lr-wpan state
void
WifiPhyStateHelper::SetLrWpanPhy (Ptr<LrWpanPhy> LrWpanPhy)
{
  NS_LOG_FUNCTION (this << LrWpanPhy);
  m_LrWpanPhy = LrWpanPhy;
  m_existLrWpanState = true;
}

//void
//syncState(LrWpanPhy &lr, WifiPhyStateHelper &wifistate){
//	TracedValue<LrWpanPhyEnumeration> lrState = lr.GetCurrentState();
//	wifistate.SetLrWpanTrxState(lrState);
//}

void
WifiPhyStateHelper::SyncLrWpanState(void)
{
  if (m_existLrWpanState){
	  m_lrwpan_trxState = m_LrWpanPhy->GetCurrentState();
  }
  else
	  m_lrwpan_trxState = IEEE_802_15_4_PHY_TRX_OFF;
}
...
bool
WifiPhyStateHelper::CheckIsLrWpanCcaBusy (void)
{
  if (m_existLrWpanState){
	  SyncLrWpanState(); //调用成功 call successfully
	  if (m_lrwpan_trxState == IEEE_802_15_4_PHY_TX_ON || m_lrwpan_trxState == IEEE_802_15_4_PHY_BUSY_TX )//||m_lrwpan_trxState==IEEE_802_15_4_PHY_BUSY||m_lrwpan_trxState==IEEE_802_15_4_PHY_RX_ON)
	  {
		  return true;}
	  else
		  return false;
...
//  if (GetState () != WifiPhyState::CCA_BUSY)
  if (GetState () != WifiPhyState::CCA_BUSY || CheckIsLrWpanCcaBusy()) //edited by weidong
    {
      m_startCcaBusy = now;
    }
  else if (m_lrwpan_trxState == IEEE_802_15_4_PHY_TX_ON){
	  m_startCcaBusy = now;
    }
