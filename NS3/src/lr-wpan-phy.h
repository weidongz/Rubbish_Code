...
public:
   ...
   *  PLME-SET-TRX-STATE.request， 由MAC子层产生，收发器状态设置请求原语，询问当前物理层信道
   *  Set PHY state
   ...
   * interconnections betweenthe PHY and the MAC. The callback
   * implement PLME CCA confirm SAP，由物理层产生，确认信道空闲或繁忙状态
   * @param c the callback
   */
  ...
  inline TracedValue<LrWpanPhyEnumeration> GetCurrentState (void) const {return m_trxState;};//edited by weidong
