...
#include "ns3/lr-wpan-phy.h" // to add LrWpanPhyEnumeration
...
public:
   ...
   * edited by Weidong
   */
  friend void syncState(LrWpanPhy &lr, const WifiPhyStateHelper &wifistate);
  ...
  /**
   * Set the lr-wpan Phy to sync the state of lrwpan
   * Edited by Weidong
   */
  void SetLrWpanPhy (Ptr<LrWpanPhy> LrWpanPhy);

  /** Edited by Weidong
   * To check lr-wpan busy state
   */
//  bool CheckLrWpanBusyState (const LrWpanPhyEnumeration lrwpan_trxState);
...
private:
  ...
  /**
   * Edited by Weidong
   * To sync the LrWpan State and WiFi CCA Busy State in order to notify the Wifi of the existence of LrWpan
   */
  ...
  /**
   * Edited by Weidong
   * The current 802.15.4 transceiver state.
   */
  TracedValue<LrWpanPhyEnumeration> m_lrwpan_trxState;
//  LrWpanPhyEnumeration m_lrwpan_CurrentState;
  Ptr<LrWpanPhy> m_LrWpanPhy;
  bool m_existLrWpanState;     //to show whether need to add LrWpanPhyState
